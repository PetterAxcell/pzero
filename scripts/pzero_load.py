...
def materialized_source(source: str) -> Iterable[DirectoryCsvSource]:
    ...
    if source_path.is_dir():
        yield DirectoryCsvSource(source_path)
        return
...
def load_postgres(args: argparse.Namespace, source: DirectoryCsvSource) -> None:
    try:
        import psycopg
...
def load_mariadb(args: argparse.Namespace, source: DirectoryCsvSource) -> None:
    try:
        import pymysql
    except ImportError as exc:  # pragma: no cover - dependency message
    ...
    try:
        with materialized_source(args.source) as source:
            if args.engine == "postgres":
                load_postgres(args, source)
            else:
                load_mariadb(args, source)
